...
async def generate_search_queries(
    ...
    try:
        ...
        # Randomization pools for varied prompts
        adjectives = ["creative", "talented", "viral", "trending", "emerging", "skilled", 
                     "professional", "authentic", "popular", "innovative", "dynamic", "engaging"]
        nouns = ["creator", "influencer", "maker", "artist", "performer", "storyteller", 
                "entertainer", "talent", "personality", "producer", "content creator"]
        verbs = ["making", "creating", "sharing", "posting", "producing", "filming", 
                "streaming", "building", "developing", "crafting"]
        styles = ["hashtag-style", "professional", "casual", "slang", "trending", 
                 "niche", "broad", "specific", "creative", "varied"]
        formats = ["1-3 words", "1-4 words", "2-5 words", "short phrases", "keywords", 
                  "phrases", "search terms", "queries"]
        
        # Random selections
        adj = random.choice(adjectives)
        noun = random.choice(nouns)
        verb = random.choice(verbs)
        style_terms = random.sample(styles, k=random.randint(2, 4))
        format_desc = random.choice(formats)
        
        # Random prompt templates (shorter)
        prompt_templates = [
            f"Generate {num_queries} TikTok search queries for {focus_area}. Use {format_desc}, mix {', '.join(style_terms)}. Return JSON with 'queries' array.",
            f"Create {num_queries} search queries to find {focus_area}. Format: {format_desc}, styles: {', '.join(style_terms)}. JSON format.",
            f"{num_queries} TikTok queries for {focus_area}. {format_desc}, include {', '.join(style_terms)}. Return JSON.",
            f"Generate {num_queries} queries ({format_desc}) for {focus_area}. Mix {', '.join(style_terms)}. JSON: 'queries' array.",
            f"TikTok search queries: {num_queries} for {focus_area}. {format_desc}, {', '.join(style_terms)}. JSON format."
        ]
        
        # Random example variations
        example_variations = [
            ['ugc creator', 'brand collaboration', 'content creator'],
            ['#ugccreator', 'brand deals', 'paid partnership'],
            ['ugc', 'influencer marketing', 'creator economy'],
            ['user generated content', 'brand partnership', 'sponsored post']
        ]
        
        # Select random prompt template and example
        selected_template = random.choice(prompt_templates)
        selected_examples = random.choice(example_variations)
        
        # Build final prompt
        prompt = f"""{selected_template}

Example: {{"queries": {json.dumps(selected_examples)}}}"""
        
        # Random system message variations (shorter)
        system_messages = [
            "Generate TikTok search queries. Return JSON with 'queries' array.",
            "Create TikTok search queries as JSON with 'queries' key.",
            "Generate TikTok queries. JSON format: 'queries' array of strings.",
            "Return TikTok search queries as JSON: 'queries' array."
        ]
        
        selected_system = random.choice(system_messages)
        
        # Random temperature (between 0.8 and 1.0)
        random_temp = round(random.uniform(0.8, 1.0), 2)
        
        # Debug: show randomization used (optional)
        print(f"🎲 Using randomized prompt (temp={random_temp}, format='{format_desc}', style={len(style_terms)} styles)")
        ...
        response = await client.chat.completions.create(
            ...
            temperature=random_temp,  # Random temperature for variation
